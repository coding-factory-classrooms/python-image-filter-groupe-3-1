...
def templateFunctionMulti(redirectory,lastImage, filter):
    for loopNumber in range(1, 4):
        if filter == "grey":
            filterName = "grey"
            optionName = cv2.cvtColor(cv2.imread(f"{redirectory}/{lastImage[loopNumber]}.jpg"), cv2.COLOR_BGR2GRAY)
        elif filter == "blur":
            filterName = "blur"
            optionName = cv2.blur(cv2.imread(f"{redirectory}/{lastImage[loopNumber]}.jpg"), sizeImage)
        elif filter == "dilate":
            filterName = "dilate"
            optionName = cv2.dilate(cv2.imread(f"{redirectory}/{lastImage[loopNumber]}.jpg"), kernel, iterations=3)
        newImage = optionName
